...
void xuLyNutNhan()
{
  ...
  if (!is_press_btn_pump && !is_press_btn_led && !is_press_btn_channel)
    return;
  ...
  // sử lý việc đổi trạng thái máy bơm nếu phát hiện ra là có người nhấn nút máy bơm
  if (is_press_btn_pump)
  {
    trangthai_pump = digitalRead(pump_in1);         // đọc trạng thái của máy bơm hiện tại.
    digitalWrite(pump_in1, !trangthai_pump);        // đảo trạng thái máy bơm
    digitalWrite(pump_in2, LOW);
    buffer_pump = String(!trangthai_pump);      // đưa vào biến tạm để tý nữa gửi lên master
    is_press_btn_pump = false;                  // reset trạng thái nút nhấn
    return;
  }
...
void xuLyLenhMaster(String cmd)
{
  ...
  // xử lý lênh khi yêu cầu mở bơm
  if (cmd == "onpump")
  {
    digitalWrite(pump_in1, HIGH);
    digitalWrite(pump_in2, LOW);
    trangthai_pump = digitalRead(pump_in1);
    buffer_pump = String(trangthai_pump);
    return;
  }
...
void layGiaTriThietBi()
{
  ...
  buffer_pump = String(digitalRead(pump_in1)) + String(digitalRead(pump_in2));
...
void loop()
{
  ...
  docDataTuMaster();
  delay(400);         //phải có delay ở đây để có thể đọc kịp dữ liệu từ ESP-Master
}